...
if (!API_KEY || API_KEY === "YOUR_API_KEY") {
  addMessage('model', 'Error: API_KEY is not configured. Please set your API key in the script.js file.');
  throw new Error("API_KEY not found or not configured.");
}
const ai = new GoogleGenAI({ apiKey: API_KEY });

const getSystemInstruction = (language) => `You are FNPH AI, a conversational AI assistant for the Federal Neuro-Psychiatric Hospital Kaduna (FNPH Kaduna). Your colors are green and yellow. Your goal is to provide information, guide users, and assist with mental and general health support in a warm and encouraging manner.
...
// --- Chat Logic ---
function hasImageInHistory(history) {
...
async function handleSendMessage(message, image = null) {
  ...
  try {
    ...
    const contentElement = modelMessageElement.querySelector('.message-content');
    const groundingChunks = new Map();
...
// --- UI Helpers ---
function createMessageElement(role, content, imageUrl) {
...
function displayUserMessage(message, imageUrl) {
...
function parseAndDisplayOptions(messageElement) {
...
function addAudioButton(messageElement, textToSpeak) {
...
function parseAndDisplayFollowUps(messageElement, questionsText) {
...
function displaySources(messageElement, sources) {
